...
#Main Screen
master = Tk()
master.title("Python Mail App with GUI")
...
def send():
    try:
        ...
        if username=="" or password=="" or to=="" or subject=="" or body=="":
            ...
            server.sendemail(username, to, finalMessage)
...
#Graphics
Label(master, text="Mail App", font=('Calibri',15)).grid(row=0, sticky=N)
Label(master, text="Use the from given below to send an Email", font=('Calibri', 11)).grid(row=1, sticky=W, padx=5)
...
recieverEntry = Entry(master, textvariable=temp_reciever)
